...
Vector serial_histogram;
vecred a_histogram = CILK_C_INIT_REDUCER(Vector, vector_reduce, vector_identity, 0, 0);
vecred c_histogram = CILK_C_INIT_COM_REDUCER(Vector, vector_reduce, vector_identity, 0);
...
void set_bins(int num) {
    ...
    CILK_C_REGISTER_COM_REDUCER(c_histogram);
}

static inline void prepare() {
    for (uint64_t i = 0; i < bins; i++) {
        REDUCER_VIEW(a_histogram).ele[i] = 0;
        COM_REDUCER_VIEW(c_histogram).ele[i] = 0;
        serial_histogram.ele[i] = 0;
    }
}

static inline void cleanup() {
    // CILK_C_UNREGISTER_REDUCER(a_histogram);
    // CILK_C_UNREGISTER_COM_REDUCER(c_histogram);
}
...
void test_reducer_serial(uint64_t * restrict arr, uint64_t num, uint64_t size) {
    for (uint64_t i = 0; i < num; i++) {
        serial_histogram.ele[arr[i % size] % bins]++;
    }
}
...
void test_reducer_commutative(uint64_t * restrict arr, uint64_t num, uint64_t size) {
    cilk_for (uint64_t i = 0; i < num; i++) {
        COM_REDUCER_VIEW(c_histogram).ele[arr[i % size] % bins]++;
    }

    COM_REDUCER_MERGE(c_histogram);
}
...
static inline uint64_t check_bins(Vector * correct, int check) {
    ...
    case 0:
        for (uint64_t i = 0; i < bins; i++) {
            if (correct->ele[i] != serial_histogram.ele[i]) return 0;
        }
        return 1;
    case 1:
        for (uint64_t i = 0; i < bins; i++) {
    ...
    case 2:
        for (uint64_t i = 0; i < bins; i++) {
            if (correct->ele[i] != COM_REDUCER_VIEW(c_histogram).ele[i]) return 0;
        }
        return 1;
    default:
        break;
